...
from src.nn_grasp_1 import SiameseEncoder, SiamesePolicyNet
...
class TrainGrasp_BC:
	...
	def forward(self, data_batch):
		...
		for trialInd in range(self.N):  # each trial in the batch is a trajectory
			...
			# A2 = A2.repeat(E1.shape[0], 1)
			dist_E1_A2 = F.pairwise_distance(E1, A2, 2) # Yantian check if it's good for LSTM embeddings. Note that it was borrowed from CNN embeddings use case
...
# Yantian
	...
	def test_success(self, epoch, path):
		...
		e_result_path = result_path + '/epoch_' + str(epoch) + '/'
		if not os.path.exists(e_result_path):
			os.umask(0)
			os.makedirs(e_result_path)

...
class GraspTeleopEnv():
	...
	def reset(self):
		...
		# self.pandaEnv.reset_arm_joints_ik(initial_ee_pos_before_depth, initial_ee_orn, fingerPos=0.04)

		# Move arm back to normal starting position
		self.pandaEnv.reset_arm_joints_ik(initial_ee_pos, initial_ee_orn, fingerPos=0.04)
	...
	def grasp_teleop(self, objX=0.55, objY=-0.05, objYaw=0, data_path='data/test/', objPath='../data/processed_objects/3DNet/100_Mug/100_Mug.urdf', numGraspPerTrial=5):
		...
		for i_episode in range(1):
			...
			np.savez(experiment_recorder.saved_dir+'/obj_configs.npz', self._obj_config)
			with open(experiment_recorder.saved_dir+'/obj_configs.txt', "w") as text_file:
				text_file.write("Position: %s \nOrientation: %s \nURDF Path: %s" % (str(self._obj_config[0]), str(self._obj_config[1]), self._obj_config[2]))
	...
	def traj_playback_and_save(self):
		with open(self.demo_path, "rb") as f:
			demos = pickle.load(f)
		...
		for d in demos:
			...
			self.pandaEnv.reset_env()
			state = self.reset()
			for step, transition in enumerate(d):
				transition = np.array(transition)
				action = transition[1]
				next_state, reward, done, _ = self.step(action)
				traj.append((state, action, reward, next_state, done))
				print("epi and step: ", step, action, done, reward)
				# step += 1
				state = next_state

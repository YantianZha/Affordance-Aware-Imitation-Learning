...
from src.experiment_record_utils import ExperimentLogger
...
class GraspTeleopEnv():
	...
	def __init__(self,
		...
		self.width = 640
		self.height = 480
		self._cam_random = 1.0

		look = [0.8, -0.1, 0.44]
		distance = 3.0
		pitch = -50 + self._cam_random * np.random.uniform(-3, 3)
		yaw = -70 + self._cam_random * np.random.uniform(-3, 3)
		roll = 0 + self._cam_random * np.random.uniform(-3, 3)
		self.viewMat = p.computeViewMatrixFromYawPitchRoll(
			look, distance, yaw, pitch, roll, 2)
		fov = 15. + self._cam_random * np.random.uniform(-2, 2)
	...
	def grasp_teleop(self, objX=0.55, objY=-0.05, objYaw=0, data_path='data/test/', objPath='../data/processed_objects/3DNet/100_Mug/100_Mug.urdf', numGraspPerTrial=5):
		...
		if self.demo_path is not None:
			self._obj_config = np.load(self.demo_path.rsplit('/',1)[0] + '/obj_configs.npz', allow_pickle=True)['arr_0']
			objPos, objOrn = self._obj_config['numbers']
			objPath = self._obj_config['strings']
		...
		objPos = np.array([objX, objY, 0.32])
		...
		for i_episode in range(1):
			...
			while not done:
				...
				while True:
					try:
						...
						print("Please use mouse to set up a proper target pose of end effector, once you're done press Ctrl+C:")
			...
			np.savez(experiment_recorder.saved_dir+'/obj_configs.npz', numbers=self._obj_config[:-1], strings=self._obj_config[-1])
			experiment_recorder.save_trajectories(SIMPLE_STATES_SIZE, fps=1, is_separated_file=True,
	...
	def get_depth(self):
		...
		# Unpack the depth image and convert to a float array
		depth_scale = p.getPhysicsEngineParameters()["fixedTimeStep"] * 1000.0
		depth_img_uint = np.array(img_arr[3]).reshape((self.height, self.width))
		depth = depth_img_uint / depth_scale
		depth = self.far * self.near / (self.far - (self.far - self.near) * depth)
	...
	def mouse_control(self, grasp, back_step=1):
		...
		actualEndEffectorOrn = quat2euler(state[5])
	
		print("Your current trans and rot: ", [actualEndEffectorPos, np.rad2deg(actualEndEffectorOrn)])
		...
		quatTarget = euler2quat(targetOrientation)

		# Calculate the difference between the quaternions
		# diff * q1 = q2  --->  diff = q2 * inverse(q1)
		DiffRotEE= quatMult(quatTarget, quatInverse(state[5]))

		TransEE = np.array(targetPosition) - actualEndEffectorPos

		RotEE = quat2euler(DiffRotEE)
		self.restore(back_step)
		print("Your current action - delta_trans, delta_rot, grasp: \n", np.concatenate((TransEE, np.rad2deg(RotEE), [grasp])))
	...
	def traj_playback(self):
		...
		# Play back
		for d in demos:
			for id, transition in enumerate(d):

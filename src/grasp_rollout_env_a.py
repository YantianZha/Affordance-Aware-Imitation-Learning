...
class GraspRolloutEnv():
	...
	def grasp(self, zs, objPos, objOrn, objPath, gui, save_figure=False, figure_path=None, mu=None, sigma=None):
		...
		for step in range(15):
			...
			# Get skill_z
			zs, A1, hidden_cell = self.encoder(inputs, 0, 0, train=False, prev_hidden_cell_lstm=hidden_cell)

			# if save_figure:
			afford_map = A1[0,-1,:,:].detach().numpy()
# 			afford_map = np.array([
...
# ])
			...
			# merge map and frame
			fin = cv2.addWeighted(afford_map, 0.7, depth_np_map, 0.3, 0)
			all_affords.append(fin)
			# cv2.imwrite(figure_path + str(step) + '.png', fin)


			# Infer action
			action = self.actor(A1, zs, state)#.squeeze(0).detach().numpy()

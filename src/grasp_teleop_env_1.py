...
class GraspTeleopEnv():
				...
				while True:
					try:
						...
						while True:
							try:
								print(
									"Press Enter if you are happy with the next state, otherwise Press # steps that you want to go back (1, 2, or 3) and then Enter to repeat\n")
								back_step = input()
	...
	def mouse_control(self, grasp, back_step=1):
		...
		actualEndEffectorOrn = p.getEulerFromQuaternion(state[5])  # R.from_quat(state[5]).as_rotvec()
		...
		try:
			while True:
				...
				# jointPoses = baxter.accurateIK(self.pandaEnv._pandaId, self._baxter.baxterEndEffectorIndex[arm], self.cid,
				# 															 targetP, p.getQuaternionFromEuler(targetO),
				# 															 self._baxter.ll, self._baxter.ul, self._baxter.jr, self._baxter.rp,
				# 															 useNullSpace=self._baxter.useNullSpace)
				# baxter.setMotors(self.pandaEnv._pandaId, jointPoses, "arm")
				# fingleValue = self._baxter.fingerDistMin if grasp[arm] > 0 else self._baxter.fingerDistMax
				# self._baxter.applyFingerAngle(fingleValue, arm)
	...
	def restore(self, id):
		# p.restoreState(self.stateId) #fileName="state.bullet")
		# print("Taking state_" + str(self._history_env_shot[-id]) + ".bullet")
		# p.restoreState(fileName="state_" + str(self._history_env_shot[-id]) + ".bullet")
    ...
    """
		...
		# state = pybullet.getLinkState(self.pandaEnv._pandaId,
		#                               self._baxter.baxterEndEffectorIndex,
		...
		#
		# 	if action[arm] > 0:
		# 		finger_angle = self._baxter.fingerDistMin if self._attempted_grasp else self._baxter.fingerDistMax
		# 		for _ in range(1000):
		# 			self._baxter.applyFingerAngle(finger_angle, arms[arm])
		# 			pybullet.stepSimulation(physicsClientId=self.cid)
		# 			finger_angle -= 0.3 / 100.
		# 			if finger_angle < 0.002:
		# 				finger_angle = 0.002
    ...
    """
		...
		# self.curr_simple_states = np.concatenate(
		# 	(pos_and_ori, end_effector_pos, end_effector_ori))  # , list(predicates.values())))
